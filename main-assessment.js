...
document.getElementById('assessmentForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    document.getElementById('assessmentSuccess').style.display = 'block';

  } catch (err) {
    console.error('Form error:', err);
    btn.disabled = false;
    btn.textContent = 'קביעת אבחון';
...
// Process + for-who cards
gsap.utils.toArray('.process-card, .for-who-item').forEach((el, i) => {
  gsap.from(el, {
    scrollTrigger: { trigger: el, start: 'top 88%', toggleActions: 'play none none none' },
    duration: 0.7, y: 50, opacity: 0, ease: 'power2.out', delay: i % 4 * 0.1
  });
});
